...
(function ($) {
	...
	if (document.location.href.indexOf('http://') != 0)	{
		path = '../script/treeTable/';
	} else {
		path = SITE_URL + 'js/treeTable/';
	}
    ...
    $.fn.treeTable = function (opts) {
        ...
        $treeTable.addChilds = function(trsHtml) {
            ...
            //modified1
            var hasChild = $('#'+pid).attr('haschild');
            var parent = $('#'+pid);
            if (hasChild == null || hasChild == 'false') {
            	parent.attr('haschild',true);
            	parent.find('span[arrow]').attr('class',css['AN']+' '+css['O']);
			}
            //modified2
            initRelation($trs);
            //modified1
            open(pid);
            parent.find('span[arrow]').removeClass(css['S']);
            parent.find('span[arrow]').addClass(css['O']);
            //modified2
        };
        //modified
        $treeTable.addParent = function(trsHtml) {
            var $trs = $(trsHtml);
            if (!$trs.length) { return false; }
